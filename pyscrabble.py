...
with open('words.txt') as words_f:
    ...
    # so that 'llab' will match different length words like 'a', 'lab', and 'ball'.
    all_perms = set()
    for length in range(1, len(in_letters) + 1):
        all_perms.update(
            set([''.join(perm) for perm in itertools.permutations(in_letters, length)])
        )

    # 4. find matches and print the result
    all_matches = set([match for match in filter(lambda perm: perm in all_words, all_perms)])
